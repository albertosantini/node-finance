...
function getOptimalPortfolio(params, callback) {
    var symbols, referenceDate, targetReturn, lows, highs;

    symbols = params.prods;
    referenceDate = params.referenceDate;
    targetReturn = params.targetReturn === 'undefined' ? undefined : params.targetReturn;
    lows = params.lows;
    highs = params.highs;
    ...
    returns.getReturns(symbols, referenceDate, function (rets) {
        ...
        if (rets.message === "") {
            ans.optim = portfolioOptim(rets.beforeRefDate, targetReturn, lows, highs);
            ans.message = ans.optim.message;
        }

        if (rets.message === "" && ans.optim.message === "") {
            if (rets.afterRefDate.length > 0 && rets.afterRefDate[0].length > 0) {
                ans.perf = performances.getPerformances(rets.afterRefDate, ans.optim.solution);
            }
        }
